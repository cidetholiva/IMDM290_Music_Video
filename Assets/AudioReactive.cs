...
public class AudioReactive : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i =0; i < numSphere; i++){
            ...
            spheres[i].transform.position += (endPosition[i] - spheres[i].transform.position) * (AudioSpectrum.audioAmp * 0.1f); //trail effect

            float scale = Mathf.Sin(time * 2f + i * 0.1f) * 0.5f + 1.5f + AudioSpectrum.audioAmp; //shape, makes breathing effect
            ...
            Renderer sphereRenderer = spheres[i].GetComponent<Renderer>();
            float hue = (float)i / numSphere; // Hue cycles through 0 to 1
            Color[] colors = { new Color(0f, 0f, 0.5f), new Color(0f, 0f, 1f), new Color(0.5f, 0.7f, 1f) }; // Dark Blue, Blue, Light Blue
            int colorIndex = (int)(time / 0.5f) % 3; // Switch colors every ~0.5s (adjust as needed)
